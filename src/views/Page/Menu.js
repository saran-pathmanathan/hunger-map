  ...
  function Menu() {
    ...
    const country = useSelector((state) => state.country.id);
    ...
    useEffect(() => {}, [header]);
    useEffect(() => {
      const changeDescription = async (country) => {
        setLoading(true);
        axios
          .get(
            `https://ml-ajkk2vma5q-ew.a.run.app/getDescription`,
            {
              params: {
                country: country,
              },
      ...
      changeDescription(country); // Always fetch a new description when the country changes
    }, [country]);
    return (
      <nav className="menu">
          ...
          <div className="legend__container">
            <div className="description__container">
              <span className="legend__labels">
                {loading ? <Loader /> : description !== "" && JSON.stringify(description.description)} 
              </span>
            </div>
          </div>
        </div>
